<template>
  ...
  <div class="absolute top-1 left-1 w-100 max-h-[calc(100vh-178px)] flex flex-col gap-1">
    <div class="container px-2! flex items-center justify-between">
      <span
        class="flex justify-center items-center gap-0.5 text-xl tracking-tighter text-shadow-[0_0_3px_#000]"
      >
        MapGenerat <Spinner /> r
      </span>
      <span class="text-xs">v{{ currentVersion }}</span>
    </div>
...
electronAPI.onCheckBeforeClose(async () => {
  if (totalLocs.value > 0) {
    return window.confirm('Are you sure you want to stop the generator ?')
  }
  return true
})
